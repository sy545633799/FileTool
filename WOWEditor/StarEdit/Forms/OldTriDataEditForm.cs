...
{
    public partial class OldTriDataEditForm : Form
    {
        OldTriDataPack pack;
        int selectId;
        int searchBase;
        ...
        {
            pack = OldTriDataPackBook.GetPack(packName);
        ...
        private void listViewIds_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (TriData bikey in pack.data.Keys)
            {
                ...
                foreach (String head in pack.header)
                {
                    ...
                    index++;
                    if (indx < pack.datatype.Count && col == 0 && (pack.datatype[indx] != "char" || pack.datasize[indx] <= 8) && (pack.datatype[indx - 1] != "char" || pack.datasize[indx - 1] <= 8))
                    {
                        col++;
                    }
                    else
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            adf.Label3 = pack.comment[2] != "" ? pack.comment[2] : pack.header[2];
            adf.ShowDialog();
            if (adf.Result == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (!pack.data.ContainsKey(bd))
                {
                    ...
                    for (int i = 3; i < pack.datatype.Count; i++)
                    {
        ...
        private void buttonImport_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                MysqlService.GetAllTriData(packName);
